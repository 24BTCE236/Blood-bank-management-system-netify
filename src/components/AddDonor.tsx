...
const AddDonor = () => {
  const { registerDonor, currentFounderId } = useBloodBank();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!currentFounderId) {
      setToast({ type: 'error', message: 'Founder access required to add donors.' });
      return;
    }
    if (!validate()) return;
    ...
    setTimeout(() => navigate('/founder-dashboard', { replace: true }), 1200);
  };
  ...
  return (
    <div className="min-h-screen bg-dashboard-radial text-slate-100 p-6">
      <div className="mx-auto max-w-2xl">
        ...
        <p className="mt-2 text-sm text-slate-300">Founders can add donors here. This page requires founder sign-in.</p>
        <form className="mt-6 grid gap-4" onSubmit={handleSubmit}>
          ...
          <div>
            <button type="submit" className="premium-button w-full">Add donor</button>
          </div>
        </form>
      </div>
      {toast ? (
